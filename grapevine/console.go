...
func (psp *PeerServerProcess) SetFailover(failover string) bool {
	return doGetAndReadResponse(psp.name, psp.host, fmt.Sprintf("/configureFailover?newfailover=%s", url.QueryEscape(failover)), "SetFailover", func(resp *http.Response) error {
		return nil
	})
}
