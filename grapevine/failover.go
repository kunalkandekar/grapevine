...
func (s *PeerServer) leafCheckinHandler(w http.ResponseWriter, req *http.Request) {
    ...
    if leafCheckin.ClusterId != s.clusterId {
        errMsg := fmt.Sprintf("%s: %s request from %v FAILED! Requested cluster %v != actual cluster %v", s.name, leafCheckin.Address, leafCheckin.ClusterId, s.clusterId)
...
func (s *PeerServer) failoverAcceptHandler(w http.ResponseWriter, req *http.Request) {
    ...
    if failoverResp.ClusterId != s.clusterId {
        errMsg := fmt.Sprintf("%s: %s request from %v FAILED! Requested cluster %v != actual cluster %v", s.name, failoverResp.Address, failoverResp.ClusterId, s.clusterId)
        http.Error(w, errMsg, http.StatusBadRequest)
        return
    }
