...
func (s *PeerServer) fwdMsg(msg *Message) error {	
    ...
    if s.parent == nil {
        log.Printf("%s: Parent is nil, forwarded from %v, possible net split! dropping %v!", s.name, msg.Forwarder, s.parent, msg)
    ...
    if msg.TTL < 1 {
        log.Printf("%s: Dropping msg due to insufficient TTL: '%v'", s.name, s.children, msg)	
        return nil
    }
    ...
    if s.chkDebugFlag(DEBUG_FLAG_FWD) && len(s.children) > 0 { log.Printf("%s: Multicasting #%05d -> %v", s.name, msg.MsgId, s.children) }    
