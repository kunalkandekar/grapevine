...
func (s *PeerServer) runConfirmLoop(joinReq *JoinRequest) {
    for {
        ...
        if oldParent != nil {
            if s.chkDebugFlag(DEBUG_FLAG_JOIN) { log.Printf("%s: Already joined %v, skipping CONFIRM to %v", s.name, oldParent, confirmHost) }
            continue
        }
    ...
    if !inmap {
    	if joinResp.Code == JOIN_RESP_CODE_ACCEPT {
    	   if isConfirm {
            ...
            s._appendLeaf(peer)
            if s.chkDebugFlag(DEBUG_FLAG_JOIN) { log.Printf("%s: %s no vacancy for %v @ %v! tryAt=%v", s.name, method, joinReq.Name, joinReq.Address, joinResp) }
        }
...
func (s *PeerServer) leaveHandler(w http.ResponseWriter, req *http.Request) {
    ...
    if leaveReq.ClusterId != s.clusterId {
        errMsg := fmt.Sprintf("%s: %s request from %v FAILED! Requested cluster %v != actual cluster %v", s.name, leaveReq.Address, leaveReq.ClusterId, s.clusterId)
